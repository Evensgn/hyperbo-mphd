...
def neg_log_marginal_likelihood_hgp_v3(mean_func,
  ...
  for dataset_key, dataset in super_dataset.items():
      ...
      for sub_dataset_key, sub_dataset in dataset.items():
        ...
        if sub_dataset.x.shape[0] == 0:
          continue
        dataset_nll += compute_nll_sub_dataset(dataset_key, sub_dataset.x, sub_dataset.y)
      total_nll += dataset_nll

  nll_data = total_nll
  nll_distribution_params = compute_nll_distribution_params()
  total_nll += nll_distribution_params

  # if return_separate_terms:
  #   return total_nll, nll_data, nll_distribution_params
  return total_nll


def neg_log_marginal_likelihood_hgp_v3_eval(mean_func,
                                       cov_func,
                                       params,
                                       super_dataset,
                                       dim_feature_values,
