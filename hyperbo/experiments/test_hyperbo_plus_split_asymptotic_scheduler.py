...
if __name__ == '__main__':
    ...
    for n_train_datasets in n_train_datasets_list:
        ...
        print('fit_gp_params_setup_a_id, n={}'.format(n_train_datasets))
        sub_process_list = []
        for seed_id in range(n_seeds):
            for train_id in train_id_list_list[seed_id]:
                new_key, key = jax.random.split(key)
                sub_process_i = subprocess.Popen([python_cmd, worker_path, '--group_id', group_id_list[seed_id],
        ...
        # alpha_mle_setup_a
        print('alpha_mle_setup_a, n={}'.format(n_train_datasets))
        sub_process_list = []
        for seed_id in range(n_seeds):
            new_key, key = jax.random.split(key)
            sub_process_0 = subprocess.Popen(
                [python_cmd, worker_path, '--group_id', group_id_list[seed_id], '--mode', 'alpha_mle_setup_a',
                 '--dataset_id', '', '--key_0', str(new_key[0]), '--key_1', str(new_key[1])])
            sub_process_0.wait()

        # eval_nll_setup_a_id (only on test datasets)
        print('eval_nll_setup_a_id, n={}'.format(n_train_datasets))
        sub_process_list = []
        for seed_id in range(n_seeds):
            for dataset_id in test_id_list:
                new_key, key = jax.random.split(key)
                sub_process_i = subprocess.Popen([python_cmd, worker_path, '--group_id', group_id_list[seed_id],
